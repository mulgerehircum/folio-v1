  ...
  return (
    <Section id="projects" className="bg-transparent" ref={sectionRef}>
      <div ref={ref} className="max-w-5xl mx-auto w-full px-6">
        ...
        >
          <h2 className="text-2xl md:text-3xl font-bold text-zinc-100">Projects</h2>
          <p className="mt-4 text-zinc-400 max-w-[65ch] text-sm leading-relaxed">
            A collection of my personal projects. Click a technology to
            highlight the projects that use it.
          </p>
        </div>

        {/* Tech Stack Filters */}
        <div
          className={`w-full mt-8 transition-all duration-400 ease-out ${
            inView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-[8px]"
          }`}
        ...
        {/* Projects Grid: 4 items = 2x2, no orphan cell */}
        <div className="mt-10 grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-12 w-full">
          {filteredProjects.map(({ project, isFiltered }, index) => {
            const delayMs = 250 + index * 70
            return (
              <div
                key={project.title}
